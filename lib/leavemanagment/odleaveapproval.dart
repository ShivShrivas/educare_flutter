...
import 'package:educareadmin/models/oddatasend.dart';
...
class OdLeaveApproval extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return OdLeaveApprovalState();
  }
}


class OdLeaveApprovalState extends State<OdLeaveApproval> {

  var colors= AppColors();
  SFData sfdata= SFData();
  var sessionId;
  var relationshipId;
  List<OdLeaveData> leavelist = <OdLeaveData>[];
  bool isLoader = false;
  final RefreshController _refreshController = RefreshController();
  var empCode,fyID,userID;
  CommonAction commonAlert= CommonAction();

  OdDataSend _odDataSend=OdDataSend();
  ...
  Future<Null> leaveList() async {
    ...
    return await api
        .listODLeave("3",relationshipId,sessionId,fyID)
        .then((result) {
      setState(() {
        isLoader = false;
  ...
  //////////////////  leave Cancel API //////////////////////
  Future<Null> leaveCancel(String transId, String status, String type) async {
    final api = Provider.of<ApiService>(context, listen: false);
    return await api
        .oDCancel("4",status,userID,jsonEncode(_odDataSend))
        .then((result) {
      setState(() {
        if(result.isNotEmpty){
          commonAlert.messageAlertSuccess(context,"OD application $type successfully","Success");
        }else{
        }
      });
    }).catchError((error) {
                ...
                child: Stack(
                    children: <Widget>[
                      ...
                      Positioned(
                        ...
                        right: 20.0,
                        child: AppBar(
                          title: Text("OD for Approvals",textAlign: TextAlign.center,style: new TextStyle(color: Colors.black,fontSize: 16.0,fontFamily: 'Montserrat',
  ...
  Future<Future<ConfirmAction?>> _asyncDeleteConfirm(BuildContext context, String transId,String type,String status, String attandanceTranid) async {
    ...
    return showDialog<ConfirmAction>(
      ...
      builder: (BuildContext context) {
        return CupertinoAlertDialog(
          title: Text('OD Approvals'),
          content:Column(
            children: [
              Text("Do you want to $type OD application?"),
          ...
          actions: <Widget>[
            ...
            FlatButton(
              ...
              onPressed: () {
                if(reasonController.text.length>1){
                  _odDataSend=new OdDataSend(approvalAuthorityRemarks:reasonController.text,transId:transId,attendanceTransId: attandanceTranid);
                  Navigator.of(context).pop(ConfirmAction.Accept);
                  leaveCancel(transId,status,type);
                }else{
                  ...
                  Expanded(
                      child: Container(
                          ...
                          child: Column(
                              ...
                              //  mainAxisAlignment: MainAxisAlignment.start,
                              children:<Widget>[
                                Text(leavelist[index].empName! == null?'':leavelist[index].empName!,
                                    maxLines: 1,
                                    // textAlign: TextAlign.justify,
                                    style: new TextStyle(color: colors.bluelight,fontSize: 12.0,fontFamily: 'Montserrat',
                                        fontWeight: FontWeight.w700)),
                                Text('Remark',
                                    maxLines: 1,
                                    // textAlign: TextAlign.justify,
                                    style: new TextStyle(color: colors.black,fontSize: 12.0,fontFamily: 'Montserrat',
                                        fontWeight: FontWeight.w700)),
                                //SizedBox(height: 2.0),
                                Text(leavelist[index].employeeRemarks! == null?'':leavelist[index].employeeRemarks!,
                                    // textAlign: TextAlign.justify,
                                    maxLines: 3,
                                ...
                                Row(
                                    ...
                                    children:<Widget>[
                                      Expanded(
                                        child: Text("In time:  ${leavelist[index].logIn! == null?'':leavelist[index].logIn!}",
                                            textAlign: TextAlign.start,
                                            style: new TextStyle(fontSize: 11.0,fontFamily: 'Montserrat',
                                                fontWeight: FontWeight.w700)),
                                      ),
                                      Expanded(
                                        child: Text("Out Time:  ${leavelist[index].logOut! == null?'':leavelist[index].logOut!}",
                                ...
                                Visibility(
                                  child:Row(
                                      ...
                                      children:<Widget>[
                                        Expanded(
                                          child: Padding(padding: const EdgeInsets.symmetric(vertical: 5.0,horizontal: 5.0),
                                              child: GestureDetector(
                                                onTap: () {
                                                   _asyncDeleteConfirm(context,leavelist[index].transId!,'Reject',"3",leavelist[index].attendanceTransId!);
                                                },
                                                child:Text("REJECT",
                                                    textAlign: TextAlign.start,
                                        ...
                                        Expanded(
                                            child: GestureDetector(
                                              onTap: () async {
                                                _asyncDeleteConfirm(context,leavelist[index].transId!,'Approve',"2",leavelist[index].attendanceTransId!);
                                              },
                                              child: Text("APPROVE",
                                                  textAlign: TextAlign.end,
                                                  style: new TextStyle(color: colors.metirialgreen,fontSize: 14.0,fontFamily: 'Montserrat',
                                                      fontWeight: FontWeight.w700)),
                                            )
                                        )
                                      ]),
