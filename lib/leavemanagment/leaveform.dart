...
class LeaveFormState extends State<LeaveForm> {
  ...
  String currentDate='',hidedate='',todate='';
  ...
  List<EmpData> userlist = <EmpData>[];
  EmpData? selectedUserCode;
 ...
 // List<String> selectedItemValue= ["Select", "Casual Leave", "Earned Leave", "Medical Leave", "Sick Leave"];
  List<String> selectedItemValueStatus=[];
  String? _currentSelectedValue;
  List<DayWiseStatus> userleavelist = <DayWiseStatus>[];
  var difference;
  var casualLeaveID,earnedLeaveID,medicalLeaveID,sickLeaveID,lwpID,selectLaeaveCode;
  var causalleaveGiven=0.0,earnedleaveGiven=0.0,medicalleaveGiven=0.0,sickleaveGiven=0.0,lwpGiven=0.0;
  var causalleaveBal,earnedleaveBal,medicalleaveBal,sickleaveBal;
  int currentIndex=5;
  List<LeaveTypeData> leaveMasterlist = <LeaveTypeData>[];
  LeaveTypeData? _leaveTypeDataModel;
  bool isLoader = false;
      ...
      if(difference<0){
        ...
        if(difference<=5){
          for(int i=0;i<difference;i++){
            ...
            print(formattedDate);
            setState(() {
              userleavelist.add(new DayWiseStatus(dateData: formattedDate,dateDataUser:formattedDateUser,abbrTypeIdEmployee: 0));
  ...
  Future<Null> leaveTypeList() async {
    ...
    return await api
        .getLeaveMasterList("4",relationshipid,sessionid,fyID)
        .then((result) {
      if(result.isNotEmpty){
        setState(() {
          leavemasterlist=result;
        });
        ...
        if(result.isNotEmpty){
          ...
          for(int i=0;i<leavelist.length;i++){
            if(leavelist[i].leaveType=='Casual Leave'){
              ...
              causalleaveBal=leavelist[i].elbalance;
            }
            if(leavelist[i].leaveType=='Earned Leave'){
              earnedLeaveID=leavelist[i].abbrTypeId;
              earnedleaveBal=leavelist[i].elbalance;
            }
            if(leavelist[i].leaveType=='Medical Leave'){
              medicalLeaveID=leavelist[i].abbrTypeId;
              medicalleaveBal=leavelist[i].elbalance;
            }
            if(leavelist[i].leaveType=='Sick Leave'){
              sickLeaveID=leavelist[i].abbrTypeId;
              sickleaveBal=leavelist[i].elbalance;
            }
            /*if(leavelist[i].leaveType=='LWP'){
              lwpID=leavelist[i].abbrTypeId;
              lwpGiven=leavelist[i].elbalance;
            }*/
          }
  ...
  Future<Null> employeeList(String sessionID, String relationshipId) async {
    ...
    final api = Provider.of<ApiService>(context, listen: false);
    return await api
        .listemployee("4",relationshipId,sessionID,fyID)
        .then((result) {
      EasyLoading.dismiss();
      if(result.table!.isNotEmpty){
        setState(() {
          userlist=result.table!;
  ...
  Future<void> saveLeave(BuildContext context,int relationshipId,int sessionID,int fyID,int userID) async {
    final api = Provider.of<ApiService>(context, listen: false);
    return await api.createLeave(empCode,leavemasterCode,designationCode,departmentCode,showdateServer,hidedateServer,reasonController.text,remarksController.text,leaveForCode,"",onleaveController.text,"","",selectEmpCode,"","",relationshipId,fyID,sessionID,userID,"1",jsonEncode(userleavelist))
        .then((result) {
      if(result.isNotEmpty){
        ...
        if(result.toString() == '"1"'){
          ...
          commonAlert.messageAlertError(context,'${result.toString()}.Please try again',"Error");
        }
      }
    }).catchError((error) {
      print("Exception");
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("Leave Application",textAlign: TextAlign.center,style: new TextStyle(color: Colors.white,fontSize: 16.0,fontFamily: 'Montserrat',
                                                  ...
                                                  //mainAxisAlignment: MainAxisAlignment.center,
                                                    children:<Widget>[
                                                      Row(
                                                          ...
                                                          children: <Widget>[
                                                            Padding(padding: const EdgeInsets.symmetric(vertical: 0.0,horizontal: 15.0),
                                                          ...
                                                          Expanded(
                                                              child: new Container(
                                                                ...
                                                                child: DropdownButton<EmpData>(
                                                                  ...
                                                                  items: this.userlist.map((EmpData data) {
                                                                    return DropdownMenuItem<EmpData>(
                                                                      child: Text(data.displayName!,style: new TextStyle(fontSize: 14.0,
                                                                          fontFamily: 'Montserrat',
                                                                          fontWeight: FontWeight.w700)),
                                                                      value: data,
                                                                    );
                                                                ...
                                                                child: Column(
                                                                   ...
                                                                   children: [
                                                                     Row(
                                                                         children: [
                                                                           Padding(padding:  new EdgeInsets.all(5.0),
                                                                             child: Text("Set Leave Type",textAlign: TextAlign.left,
                                                                          ...
                                                                          child: Column(
                                                                              ...
                                                                              children:<Widget>[
                                                                                Row(
                                                                                  children: [
                                                                                    Padding(padding:  new EdgeInsets.all(5.0),
                                                                                      child: Text("Leave Type",textAlign: TextAlign.left,
                                                                                          style: new TextStyle(color: colors.black,fontSize: 14.0,fontFamily: 'Montserrat',
                                                                                              fontWeight: FontWeight.w700)),
                                                                                    ),
                                                                                  ],
                                                                                  ...
                                                                                  children: <Widget>[
                                                                                    Expanded(
                                                                                        child: new Container(
                                                                                          ...
                                                                                          child: DropdownButton<LeaveTypeMaster>(
                                                                                            isExpanded: true,
                                                                                            value: leavemaster,
                                                                                            icon: Icon(Icons.arrow_drop_down),
                                                                                            iconSize: 24,
                                                                                            ...
                                                                                            style: TextStyle(color: Colors.black, fontSize: 18),
                                                                                            underline: SizedBox(),
                                                                                            onChanged: (LeaveTypeMaster? data) {
                                                                                              setState(() {
                                                                                                leavemaster = data;
                                                                                                laveMasterCode=leavemaster!.abbrTypeId;
                                                                                                // print(laveMasterCode);
                                                                                              });
                                                                                            },
                                                                                            items: this.leavemasterlist.map((LeaveTypeMaster data) {
                                                                                              return DropdownMenuItem<LeaveTypeMaster>(
                                                                                                child: Text("${data.abbrType} (${data.totalLeaves})",style: new TextStyle(fontSize: 14.0,
                                                                                                    fontFamily: 'Montserrat',
                                                                                                    fontWeight: FontWeight.w700),),
                                                                                                value: data,
                                                                                              );
                        ...
                        child: Expanded(
                            child: new Container(
                              ...
                              child: DropdownButton(
                                isExpanded: true,
                                hint: Text('Select',style: new TextStyle(fontSize: 11.0,
                                    fontFamily: 'Montserrat',
                                    fontWeight: FontWeight.w700)),
                                //isDense: true,
                                 ...
                                 if(selectLaeaveCode==null){
                                   ...
                                   setState(() {
                                     ...
                                     for(int i=0;i<userleavelist.length;i++){
                                       ...
                                       if(userleavelist[i].abbrTypeIdEmployee==null){
                                         selectLaeaveCode=null;
                                       }
                                     }
